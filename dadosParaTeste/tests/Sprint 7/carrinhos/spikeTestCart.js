...
const cartEndpoint = `${ENDPOINTS.CARTS_ENDPOINT}/cancelar/compra`;

export default function (data) {
    let iteration = __ITER;
    try {
        const authToken = data[iteration].authToken;
        const delRes = baseRest.del(cartEndpoint, null, { headers: { 'Authorization': `${authToken}` } });
...
export function teardown(data) {
    try {
        for (let i = 0; i < data.length; i++) {
            ...
            if (!authToken) {
                console.error(`Token de autenticação não encontrado para o índice ${i}`);
                continue;
            }
